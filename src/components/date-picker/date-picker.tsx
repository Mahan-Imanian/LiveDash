...
export function DatePicker({
	...
	const [currentDate, setCurrentDate] = useState<moment.Moment>(
		selectedDate?.clone().locale('en') || moment().locale('en')
	)
	...
	const isCurrentMonthToday = () => {
		const today = moment()
		return currentDate.month() === today.month() && currentDate.year() === today.year()
	}
	...
	const changeMonth = (delta: number) => {
		setCurrentDate((prev) => prev.clone().add(delta, 'month'))
	}
	...
	const handleDateClick = (
		...
		if (isCurrentMonth) {
			targetDate = currentDate.clone().date(day)
		} else if (isPrevMonth) {
			targetDate = currentDate.clone().subtract(1, 'month').date(day)
		} else {
			targetDate = currentDate.clone().add(1, 'month').date(day)
		}
	...
	const renderDay = (
		...
		let cellDate: moment.Moment

		if (isCurrentMonth) {
			cellDate = currentDate.clone().date(day)
		} else if (isPrevMonth) {
			cellDate = currentDate.clone().subtract(1, 'month').date(day)
		} else {
			cellDate = currentDate.clone().add(1, 'month').date(day)
		}
		...
		const getDayTextStyle = () => {
			if (isDaySelected) return 'bg-primary text-white font-medium'
			if (isWeekend) return 'text-error bg-error/10'
			if (!isCurrentMonth) return 'text-muted opacity-50'
			return 'text-content hover:bg-base-300'
		}

		const getHoverStyle = () => {
			if (isDaySelected) return ''
			if (isWeekend) return 'hover:bg-red-400/10'
			return 'hover:bg-primary/10'
		}

		const getTodayRingStyle = () => {
			if (isDaySelected) return ''
			if (isWeekend) return 'border border-dashed border-error/80'
