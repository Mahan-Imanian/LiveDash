			...
			<div className="flex flex-col max-h-[80vh]">
				<div className="flex items-center justify-between px-1 pb-1">
					<div className="flex flex-col">
						<p className="mt-1 text-xs font-medium text-muted">
							New update for LiveDash’s first anniversary 🎂
						</p>
					</div>
				</div>
				...
				<div className="pb-2 space-y-4 overflow-y-auto h-110">
					...
					<div className="p-4 space-y-3 border rounded-2xl bg-base-300/20 border-base-300/10">
						...
						<p className="text-xs leading-6 text-muted">
							...
							<span className="font-bold text-content">
								fresh look and layout
							</span>{' '}
							was added to LiveDash. The previous interface is still available, and now you can
							switch between two styles and choose the one that fits your workflow.
							
						</p>
						<ul className="space-y-1">
							{[
								'The previous interface is still available',
								'A new interface with a different feel',
								'Quick switch from Settings -> Appearance',
							].map((item, i) => (
								<li
									key={i}
					...
					<div className="flex items-center justify-center gap-2 py-1 text-muted">
						...
						<span className="text-xs">Thanks for being with us</span>
					</div>
				</div>
				...
				<div className="flex items-center justify-between px-4 py-2 border border-t border-base-300/10 bg-base-200/40 rounded-3xl">
					...
					<Button
						...
						isPrimary={true}
					>
						{counter > 0 ? `Wait a moment (${counter})` : 'Got it'}
					</Button>
				</div>
			</div>
		</Modal>
