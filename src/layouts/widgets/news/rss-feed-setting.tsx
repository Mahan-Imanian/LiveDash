...
export const RssFeedSetting = () => {
	...
	const addSuggestedFeed = (suggestedFeed: { name: string; url: string }) => {
		...
		if (feedExists) {
			setError(`Feed "${suggestedFeed.name}" has already been added.`)
			return
		}
	...
	return (
		<WidgetSettingWrapper>
			...
			<SectionPanel title="General settings" size="sm">
				<CheckBoxWithDescription
					isEnabled={rssState.useDefaultNews}
					onToggle={toggleDefaultNews}
					title="Use default news sources"
					description="When enabled, news from default sources is shown."
				/>
			</SectionPanel>

			<SectionPanel title="Add new RSS feed" size="sm">
				<div className="flex flex-col gap-3">
					<TextInput
						type="text"
						placeholder="Feed name (example: TechCrunch)"
						value={newFeed.name}
						onChange={(value) => setNewFeed({ ...newFeed, name: value })}
					/>
					<TextInput
						type="url"
						placeholder="RSS URL (example: https://www.euronews.com/rss)"
						value={newFeed.url}
						onChange={(value) => {
			...
			<SectionPanel title="Suggested feeds" size="sm">
			...
			<SectionPanel title={`Your feeds (${rssState.customFeeds.length})`} size="sm">
				<FeedsList
			...
			{feeds.length === 0 ? (
				...
				>
					...
					<p className={'mb-1 text-sm font-medium opacity-70 text-content'}>
						No RSS feed has been added.
					</p>
					<p className={'text-xs opacity-50'}>
						Use the form above to add a feed.
					</p>
				</div>
			) : (
