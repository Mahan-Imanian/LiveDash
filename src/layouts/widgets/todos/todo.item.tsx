...
export function TodoItem({
	...
	const handleDelete = (e: React.MouseEvent) => {
		if (isTemp) return showToast('This task has not been synced yet.', 'error')
		e.stopPropagation()
		if (isPending) return
		if (!isAuthenticated) return showToast('Sign in to delete.', 'error')
	...
	const handleEdit = (e: React.MouseEvent) => {
		if (isTemp) return showToast('This task has not been synced yet.', 'error')
		e.stopPropagation()
		if (!isAuthenticated) return showToast('Sign in to edit.', 'error')
	...
	const onConfirmDelete = async () => {
		...
		if (validate(onlineId)) return showToast('Task ID error', 'error')
	...
	const onToggleClick = async (e: React.MouseEvent) => {
		...
		if (!isAuthenticated) {
			return showToast('Sign in to change task status.', 'error')
		}
				...
				<div className="border-t border-base-content/5 bg-base-content/1 px-2.5 py-2">
					...
					<div className="flex items-center gap-2 text-[10px]">
						...
						<span className="flex items-center gap-1 mr-auto text-muted/60">
							...
							{parseTodoDate(todo.date).locale('en').format('D MMMM')}
						</span>
					</div>
