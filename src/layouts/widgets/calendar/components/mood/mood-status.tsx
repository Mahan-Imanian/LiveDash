...
export const moodOptions = [
	{ value: 'sad', emoji: '😔', label: 'Sad', colorClass: 'bg-error/20' },
	{ value: 'tired', emoji: '😴', label: 'Tired', colorClass: 'bg-warning/20' },
	{ value: 'happy', emoji: '🙂', label: 'Okay', colorClass: 'bg-secondary/20' },
	{ value: 'excited', emoji: '😄', label: 'Energized', colorClass: 'bg-success/20' },
]
		...
		<div className="flex flex-col h-full gap-1 p-2">
			...
			<div className="grid grid-cols-7 gap-[2px] mt-2">
				{days.map((day: any, i: number) => {
					...
					return (
						<Tooltip
							content={moodOpt?.label || 'Not submitted'}
							key={`user-mood-${i}`}
						>
							<div
								title={`${day.date} - ${moodOpt?.label ?? 'Not submitted'}`}
								className={`
