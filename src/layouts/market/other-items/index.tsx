...
export function MarketOtherItems() {
	...
	const handlePurchaseClick = (item: MarketItem) => {
		if (!isAuthenticated) {
			...
			showToast('Sign in to buy items.', 'error')
			return
		}
		...
		return (
			<div className="flex flex-col items-center justify-center h-64 text-center">
				...
				<p className="text-lg font-medium text-content">
					Error loading market items
				</p>
				<p className="text-sm text-muted">Try again.</p>
			</div>
		)
	}
		...
		<>
			{isLoading ? (
				...
				<div className="flex flex-col items-center justify-center h-64 text-center">
					...
					<p className="text-sm text-muted">Check again later</p>
				</div>
			)}
