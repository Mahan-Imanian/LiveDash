...
export function MarketItemPurchaseModal({
	...
	const handlePurchase = () => {
		...
		purchaseItem(
			...
			{
				onSuccess: (_response) => {
					showToast(`${item.name} was purchased successfully. 🎉`, 'success', {
						alarmSound: true,
					})
				...
				onError: (error) => {
					showToast(
						(translateError(error) as string) || 'Error buying item',
						'error'
					)
			...
			<div className="space-y-4">
				...
				<div className="p-3 space-y-2 border rounded-xl border-primary/20 bg-primary/5">
					<div className="flex items-center justify-between">
						<span className="text-sm text-content">Current balance:</span>
						<UserCoin coins={userCoins} />
					</div>
					<div className="flex items-center justify-between">
						<span className="text-sm text-content">Item price:</span>
						...
						<div className="flex items-start gap-2">
							...
							<div>
								...
								<p className="text-xs text-warning/80">
									You need {item.price - userCoins} LiveCoin
