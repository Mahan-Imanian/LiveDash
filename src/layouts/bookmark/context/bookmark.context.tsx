...
export const BookmarkProvider: React.FC<{ children: React.ReactNode }> = ({
	...
	const addBookmark = async (
		...
		if (!isAuthenticated)
			return showToast('Sign in to add a bookmark.', 'error')

		try {
			if (inputBookmark.icon && inputBookmark.icon.size > MAX_ICON_SIZE) {
				showToast(
					`Selected image (${(inputBookmark.icon.size / (1024 * 1024)).toFixed(1)} MB) is larger than the allowed maximum.`,
					'error'
				)
				cb()
				return
	...
	const editBookmark = async (input: BookmarkUpdateFormFields, cb: () => void) => {
		...
		if (!foundedBookmark) return showToast('Bookmark not found.', 'error')
		...
		if (!bookmarkIdToEdit || validate(bookmarkIdToEdit)) {
			showToast(
				'Sync your bookmarks before editing this bookmark.',
				'error',
				{
	...
	const deleteBookmark = async (id: string, cb: () => void) => {
		...
		if (validate(idToDelete)) {
			showToast(
				'Sync your bookmarks before deleting this bookmark.',
				'error',
				{
					duration: 8000, // 8 seconds
				}
