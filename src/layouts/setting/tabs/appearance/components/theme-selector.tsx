...
const defaultThemes: ThemeItem[] = [
	{
		...
		description: 'Transparent theme with a glass effect',
	},
	{
		id: 'icy',
		name: 'Ice',
		icon: <MdOutlineBlurOn size={14} />,
		description: 'Transparent white theme with an icy look',
	},
	{
		id: 'light',
		name: 'Light',
...
export function ThemeSelector({ fetched_themes }: Props) {
	...
	useEffect(() => {
		if (fetched_themes.length) {
			const mapped: ThemeItem[] = fetched_themes.map((item) => ({
				...
				icon: <IoMdStar size={14} />,
				description: item?.description || 'Purchased theme',
			}))
	...
	return (
		<SectionPanel title="Choose theme" delay={0.2} size="sm">
			<div className="space-y-3">
				<p className="text-sm text-muted">Choose the LiveDash appearance theme.</p>
