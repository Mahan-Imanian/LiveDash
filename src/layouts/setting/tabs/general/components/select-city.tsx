...
export function SelectCity({ onSave, size }: Prop) {
	...
	return (
		<SectionPanel title="Select city" size={size ? size : 'sm'}>
			<div className="space-y-2">
				<button
					onClick={onModalOpen}
					disabled={isSettingCity}
					className="flex items-center justify-between w-full p-3 text-left transition-colors border cursor-pointer rounded-2xl bg-base-100 border-base-300 hover:bg-base-200 disabled:opacity-50 disabled:cursor-not-allowed"
				>
					{isLoadingUser ? (
				...
				{error && (
					<div className="p-3 text-sm text-left duration-300 border rounded-lg border-error/20 bg-error/10 backdrop-blur-sm animate-in fade-in-0">
			...
			{showAuthModal && (
				<AuthRequiredModal
					...
					message="Sign in to select a city."
				/>
			)}
			<Modal
				isOpen={isModalOpen}
						...
						{isLoading ? (
							...
							filteredCities.map((city) => (
								<div
									...
									className="flex items-center w-full p-3 text-left transition-all duration-200 border-b cursor-pointer border-base-200/30 last:border-b-0 group rounded-2xl hover:bg-primary/20 hover:text-primary"
								>
