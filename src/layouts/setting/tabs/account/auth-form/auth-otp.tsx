		...
		try {
			...
			if (typeof content === 'string') {
				if (err.response?.data?.message === 'INVALID_OTP_CODE') {
					setError({
						email: null,
						otp: 'The verification code is invalid. Try again.',
						api: null,
					})
				...
				if (Object.keys(content).length === 0) {
					setError({
						email: null,
						otp: null,
						api: 'The verification code is invalid. Try again.',
					})
					return
				}
				setError({
