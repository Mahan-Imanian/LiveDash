...
const getGenderInfo = (gender: 'MALE' | 'FEMALE' | 'OTHER' | null | undefined) => {
	if (gender === 'MALE') return { label: 'Male' }
	if (gender === 'FEMALE') return { label: 'Female' }
	return { label: 'Prefer not to say' }
}

const formatGregorianDate = (dateString: string | null | undefined): string => {
	if (!dateString) return 'Not set'
	try {
		const gregorianDate = moment(dateString, 'YYYY-MM-DD')
		return gregorianDate.isValid()
			? gregorianDate.locale('en').format('D MMMM YYYY')
			: dateString
	} catch {
		return dateString || '-'
	}
		...
		<div className="flex flex-col space-y-4">
			<div className="relative flex flex-row items-center justify-between p-2 overflow-hidden border bg-base-100/50 border-content rounded-3xl">
				...
				<div className="flex flex-col items-end gap-2">
					...
					<div className="text-xs font-medium opacity-70 mb-0.5">
						<span>
							Joined in{' '}
							{moment(user?.joinedAt).locale('en').format('MMMM YYYY')}
						</span>
					</div>
				</div>
			</div>
			...
			<div className="overflow-hidden border border-base-300/50 rounded-2xl bg-base-100/30">
				...
				<DisplayRow
					...
					label="Birth date"
					value={formatGregorianDate(user?.birthDate)}
...
const DisplayRow = ({
	...
	return (
		<div className="flex items-center justify-between p-2 transition-colors border-b last:border-b-0 border-base-300/30 hover:bg-base-200/20">
			...
			<div
				className={`relative flex justify-end text-xs w-fit pr-1 font-semibold text-content ${isLtr ? 'dir-ltr' : 'dir-ltr'}`}
			>
