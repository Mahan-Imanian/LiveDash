...
export const SettingsDropdown = ({ setShowSettings }: SettingsProps) => {
	...
	const onClick = () => {
		if (ui === 'SIMPLE') {
			showToast('Widgets cannot be changed or moved in simple appearance mode.', 'error')
			return
		}
	...
	const content = (
		<div className="py-2 bg-content bg-glass min-w-[200px] rounded-2xl">
			<button
				...
				className="flex items-center w-full gap-3 px-3 py-2 text-sm text-left transition-colors rounded-none cursor-pointer group hover:bg-primary/10 hover:text-primary"
			>
			...
			<button
				...
				className="flex items-center justify-between w-full px-3 py-2 text-sm text-left transition-colors rounded-none cursor-pointer group hover:bg-primary/10 hover:text-primary"
			>
			...
			>
				<div className="flex items-center gap-3">
					...
					/>
					{canReOrderWidget ? (
						<span>Disable move mode</span>
					) : (
						<span>Widget move mode</span>
					)}
				</div>
			</div>
		</div>
	...
	return (
		<>
			<div
				ref={triggerRef}
				className="relative p-2 transition-all cursor-pointer nav-btn text-white/70 hover:text-white active:scale-90"
				id="settings-button"
			>
				<HiCog size={20} />
			</div>
