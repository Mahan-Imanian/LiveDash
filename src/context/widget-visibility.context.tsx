...
export const widgetItems: WidgetItem[] = [
	...
	},
	{
		...
		emoji: '🔗',
		label: 'Combo widget (Rates and News)',
		order: 4,
		node: (
			<CurrencyProvider>
				<ComboWidget />
		...
		setVisibility((prev) => {
			...
			if (!isCurrentlyVisible) {
				if (!isAuthenticated && prev.length >= MAX_VISIBLE_WIDGETS) {
					showToast(
						`Guest users can only enable up to ${MAX_VISIBLE_WIDGETS} widgets. Sign in to enable more widgets.`,
						'error'
					)
					return prev
				}
