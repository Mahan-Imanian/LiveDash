(function () {
  ...
  const icons = {
    home: svg('<path d="M4 11.2 12 4l8 7.2V20h-5v-5H9v5H4v-8.8Z" fill="currentColor"/>'),
    apps: svg('<path d="M5 5h6v6H5V5Zm8 0h6v6h-6V5ZM5 13h6v6H5v-6Zm8 0h6v6h-6v-6Z" fill="currentColor"/>'),
    explore: svg('<circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" stroke-width="2"/><path d="m15 9-2.2 5.8L9 16l2.2-5.8L15 9Z" fill="currentColor"/>'),
    settings: svg('<circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="2"/><path d="M12 4v2m0 12v2m8-8h-2M6 12H4m13.7-5.7-1.4 1.4M7.7 16.3l-1.4 1.4m11.4 0-1.4-1.4M7.7 7.7 6.3 6.3" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'),
    user: svg('<circle cx="12" cy="8" r="3.5" fill="none" stroke="currentColor" stroke-width="2"/><path d="M5.5 19c1.3-3.3 3.5-5 6.5-5s5.2 1.7 6.5 5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'),
    sync: svg('<path d="M17 8a6 6 0 0 0-10.2 3.2M7 8H4V5m3 11a6 6 0 0 0 10.2-3.2M17 16h3v3" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>'),
    x: svg('<path d="m7 7 10 10M17 7 7 17" stroke="currentColor" stroke-width="2.3" stroke-linecap="round"/>'),
    chevron: svg('<path d="m7 10 5 5 5-5" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/>'),
    search: svg('<circle cx="11" cy="11" r="5.8" fill="none" stroke="currentColor" stroke-width="2"/><path d="m16 16 4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'),
    plus: svg('<path d="M12 5v14M5 12h14" stroke="currentColor" stroke-width="2.3" stroke-linecap="round"/>'),
    check: svg('<path d="m5 12 4.2 4.2L19 6.5" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/>'),
    clock: svg('<circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" stroke-width="2"/><path d="M12 7v5l3 2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'),
    cloud: svg('<path d="M8 17h8a4 4 0 0 0 .4-8 5 5 0 0 0-9.7 1.4A3.4 3.4 0 0 0 8 17Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/>'),
    task: svg('<path d="M8 7h11M8 12h11M8 17h11" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="m4 7 .8.8L6.5 6M4 12l.8.8 1.7-1.8M4 17l.8.8 1.7-1.8" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/>'),
    note: svg('<path d="M7 4h8l4 4v12H7V4Z" fill="none" stroke="currentColor" stroke-width="2"/><path d="M15 4v4h4M10 12h5M10 16h4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'),
    feed: svg('<path d="M7 11h10v3a5 5 0 0 1-10 0v-3Z" fill="none" stroke="currentColor" stroke-width="2"/><path d="M8 8c1-1 2-1 3 0s2 1 3 0 2-1 3 0" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'),
    ball: svg('<circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" stroke-width="2"/><path d="M4.5 10.5c3.5 1.2 6.7.4 9.8-2.5M9 19c-.8-3.4.1-6.6 2.7-9.8" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>'),
    rest: svg('<path d="M8 7h8M8 12h6M8 17h10" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'),
    google: svg('<path d="M12 4.5a7.5 7.5 0 1 0 0 15c3.9 0 6.8-2.6 6.8-6.2 0-.5 0-.8-.1-1.2H12v2.7h3.9c-.3 1.6-1.7 2.7-3.9 2.7a4.9 4.9 0 1 1 3.4-8.4l2-2A7.4 7.4 0 0 0 12 4.5Z" fill="currentColor"/>'),
    lock: svg('<rect x="6" y="10" width="12" height="9" rx="2" fill="none" stroke="currentColor" stroke-width="2"/><path d="M8.5 10V8a3.5 3.5 0 0 1 7 0v2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>')
  };

  const brandMap = {
    gmail: ['gmail', 'M'], calendar: ['calendar', '31'], 'google calendar': ['calendar', '31'], drive: ['drive', '▲'], 'google drive': ['drive', '▲'], notion: ['notion', 'N'], todoist: ['todoist', '✓'], slack: ['slack', 'S'], zoom: ['zoom', 'Z'], dropbox: ['dropbox', '□'], outlook: ['outlook', 'O'], 'microsoft to do': ['todo', '✓'], '1password': ['onepassword', icons.lock], linkedin: ['linkedin', 'in'], github: ['github', '{}'], figma: ['figma', 'F'], canva: ['canva', 'C'], spotify: ['spotify', '♬'], youtube: ['youtube', '▶'], wikipedia: ['wikipedia', 'W'], irs: ['irs', 'US'], 'usps tracking': ['usps', 'US'], 'gov.uk': ['govuk', 'GB'], 'eu portal': ['eu', 'EU'], nhs: ['nhs', 'NH'], 'dhl tracking': ['dhl', 'D'], 'royal mail': ['royalmail', 'RM'], ups: ['ups', 'UPS'], trello: ['trello', 'T'], miro: ['miro', 'M'], loom: ['loom', 'L'], tinypng: ['tinypng', '🐼'], cloudconvert: ['cloudconvert', '☁'], 'remove.bg': ['removebg', '✂'], unsplash: ['unsplash', 'U'], 'google translate': ['translate', 'G'], speedtest: ['speedtest', '⚡'], 'archive.org': ['archive', 'A'], search: ['google', 'G'], maps: ['maps', '⌖'], docs: ['docs', 'D'], sheets: ['sheets', 'S'], slides: ['slides', 'P'], meet: ['meet', 'M'], keep: ['keep', 'K'], news: ['news', 'N'], photos: ['photos', 'P'], chatgpt: ['chatgpt', '✦'], claude: ['claude', 'C'], perplexity: ['perplexity', 'P'], gemini: ['gemini', '◇'], copilot: ['copilot', 'C'], 'hugging face': ['huggingface', '🤗'], runway: ['runway', 'R'], midjourney: ['midjourney', 'M'], 'google flights': ['flights', '✈'], 'booking.com': ['booking', 'B'], airbnb: ['airbnb', 'A'], uber: ['uber', 'U'], wise: ['wise', 'W'], revolut: ['revolut', 'R'], 'yahoo finance': ['finance', 'Y'], 'xe currency': ['currency', '$'], facebook: ['facebook', 'f'], instagram: ['instagram', '◎'], x: ['xbrand', 'X'], reddit: ['reddit', 'r'], discord: ['discord', 'D'], whatsapp: ['whatsapp', '☎'], telegram: ['telegram', '✈'], pinterest: ['pinterest', 'P'], mail: ['gmail', 'M']
  };

  function miniIcon(key) { return `<span class="wg-mini-icon" aria-hidden="true">${icons[key] || icons.apps}</span>`; }
  function normalize(value) { return String(value || '').trim().toLowerCase(); }
  function host(url) { try { return new URL(/^https?:\/\//i.test(url || '') ? url : `https://${url}`).hostname.replace(/^www\./, ''); } catch { return ''; } }
  function appBrand(app) {
    const name = normalize(app?.name || app?.label || '');
    const domain = normalize(host(app?.url || ''));
    const key = brandMap[name] || brandMap[domain.replace(/\.com$|\.org$|\.net$/g, '')] || brandMap[domain] || ['default', (app?.name || app?.label || 'A').slice(0, 2).toUpperCase()];
    return key;
  }
  function appIcon(app, cls = '') {
    const [brand, glyph] = appBrand(app);
    return `<span class="app-mark ${escapeHtml(cls)} brand-${escapeHtml(brand)}"><span>${glyph}</span></span>`;
  }
  ...
  function toast(message) { let wrap = $('.toast-wrap'); if (!wrap) { wrap = document.createElement('div'); wrap.className = 'toast-wrap'; document.body.appendChild(wrap); } const el = document.createElement('div'); el.className = 'toast'; el.textContent = message; wrap.appendChild(el); setTimeout(() => el.remove(), 2600); }
  ...
  let syncTimer = null;
  function scheduleSync(reason) { if (syncTimer) clearTimeout(syncTimer); syncTimer = setTimeout(() => syncCloud(reason), 900); }
  ...
  function renderProfileBar() {
    ...
    return `<header class="top-shell"><button class="widgetify-logo" data-route="home" type="button"><img src="assets/icons/icon48.png" alt=""><span>LiveDash</span></button><div class="top-status ${isCloud ? 'signed' : ''}"><span></span>${isCloud ? `Cloud active · ${escapeHtml(state.profile.email || 'Profile synced')}` : 'Local dashboard · Sign in for cloud sync'}</div><button class="profile-nav ${isCloud ? 'signed' : ''}" data-action="account" type="button"><span class="profile-avatar">${avatar}</span><span><strong>${isCloud ? escapeHtml(state.profile.name || state.profile.email || 'Profile') : 'Sign in'}</strong><small>${isCloud ? 'Cloud profile' : 'Local mode'}</small></span></button></header>`;
  }
  function renderTabs() { return `<nav class="category-tabs" aria-label="Categories">${(state.categories || []).map((c) => `<button class="category-tab ${state.settings.appCategory === c.id ? 'active' : ''}" data-action="category" data-category="${escapeHtml(c.id)}" type="button"><span>${appIcon({ name: c.label, url: c.id }, 'tiny')}</span><b>${escapeHtml(c.label)}</b></button>`).join('')}</nav>`; }
  ...
  function renderRates() { return `<section class="wigi-card rates-widget"><div class="widget-head"><div>${miniIcon('sync')}<span><strong>Rates</strong><small>Base ${escapeHtml(state.settings.currencyBase || 'USD')}</small></span></div><button data-action="refresh" type="button">${miniIcon('sync')}</button></div><div class="rate-list">${(state.currency || []).map((c) => `<div><span>${escapeHtml(c.code)}</span><strong>${escapeHtml(c.value)}</strong><em>${c.delta === 'flat' ? '→' : '↑'}</em></div>`).join('')}</div></section>`; }
  ...
  function renderPet() {
    ...
    const caption = mode === 'play' ? 'Akita is chasing the ball' : mode === 'feed' ? 'Akita is eating' : mode === 'rest' ? 'Akita is resting' : 'Click the ball to play';
    return `<section class="wigi-card akita-widget pet-${escapeHtml(mode)}"><div class="widget-head"><div>${miniIcon('ball')}<span><strong>Akita</strong><small>${escapeHtml(pet.mood || 'Ready')} · Score ${Number(pet.score || 0)}</small></span></div></div><button class="akita-stage" data-action="pet-play" type="button" aria-label="Throw ball to Akita"><img src="${escapeHtml(petSprite())}" alt="Akita dog"><span class="akita-ground"></span><span class="akita-caption">${caption}</span><i class="akita-ball" aria-hidden="true"></i></button><div class="pet-controls"><button data-action="pet-feed" type="button">${miniIcon('feed')}Feed</button><button data-action="pet-play" type="button">${miniIcon('ball')}Throw ball</button><button data-action="pet-rest" type="button">${miniIcon('rest')}Rest</button></div></section>`;
  }
  function renderTasks() {
    const tasks = (state.tasks || []).filter((t) => activeTaskFilter === 'all' ? true : activeTaskFilter === 'done' ? t.status === 'done' : t.status !== 'done').slice(0, 6);
    const counts = { open: (state.tasks || []).filter((t) => t.status !== 'done').length, done: (state.tasks || []).filter((t) => t.status === 'done').length };
    return `<section class="wigi-card task-widget"><div class="task-top"><div>${miniIcon('task')}<span><strong>Tasks</strong><small>${counts.open} open · ${counts.done} done</small></span></div><button data-action="add-task" type="button">Add</button></div><div class="task-tabs"><button class="${activeTaskFilter === 'open' ? 'active' : ''}" data-action="task-filter" data-filter="open" type="button">Open</button><button class="${activeTaskFilter === 'all' ? 'active' : ''}" data-action="task-filter" data-filter="all" type="button">All</button><button class="${activeTaskFilter === 'done' ? 'active' : ''}" data-action="task-filter" data-filter="done" type="button">Done</button></div><div class="todo-list">${tasks.length ? tasks.map((t) => `<article class="todo-item ${t.status === 'done' ? 'done' : ''}"><button class="todo-check" data-action="complete-task" data-id="${escapeHtml(t.id)}" type="button" aria-label="Toggle task">${t.status === 'done' ? miniIcon('check') : ''}</button><div class="todo-copy"><strong>${escapeHtml(t.title)}</strong><small><span class="priority-${escapeHtml(t.priority || 'medium')}">${escapeHtml(t.priority || 'medium')}</span> · ${escapeHtml(t.source || 'LiveDash')}</small></div><button class="todo-delete" data-action="delete-task" data-id="${escapeHtml(t.id)}" type="button" aria-label="Delete task">${miniIcon('x')}</button></article>`).join('') : '<div class="task-empty">No tasks here.</div>'}</div><div class="task-compose"><input id="taskInput" placeholder="Add a useful task..." aria-label="Task title"><button data-action="add-task-input" type="button">${miniIcon('plus')}</button></div></section>`;
  }
  function renderFocus() { const r = state.focus.remaining || (state.settings.focusMinutes || 25) * 60; const total = (state.focus.mode === 'break' ? 5 : state.settings.focusMinutes || 25) * 60; const pct = Math.max(0, Math.min(1, 1 - (r / total))); return `<section class="wigi-card focus-widget"><div class="widget-head"><div>${miniIcon('clock')}<span><strong>Focus</strong><small>${state.focus.running ? 'Running' : 'Ready'}</small></span></div></div><div class="pomo-ring" style="--pomo:${Math.round(pct * 360)}deg"><strong>${Math.floor(r / 60).toString().padStart(2, '0')}:${(r % 60).toString().padStart(2, '0')}</strong><span>${escapeHtml(state.focus.mode || 'work')}</span></div><div class="focus-controls"><button data-action="pomo-reset" type="button">Reset</button><button class="primary-action" data-action="pomo-toggle" type="button">${state.focus.running ? 'Pause' : 'Start'}</button><button data-action="pomo-mode" type="button">Mode</button></div></section>`; }
  ...
  function renderExplore() { return `<main class="explore-page"><section class="app-panel"><div class="panel-title"><strong>Notes</strong><button data-action="add-note" type="button">New note</button></div><div class="note-list">${(state.notes || []).slice(0, 8).map((n) => `<article><strong>${escapeHtml(n.title)}</strong><span>${escapeHtml(n.tag || 'note')} · ${escapeHtml(new Date(n.createdAt).toLocaleDateString())}</span></article>`).join('')}</div></section><section class="app-panel"><div class="panel-title"><strong>World clocks</strong></div><div class="rate-list">${(state.worldClocks || []).map((c) => `<div><span>${escapeHtml(c.city)}</span><strong>${escapeHtml(cityTime(c.offset))}</strong><em></em></div>`).join('')}</div></section></main>`; }
  ...
  function renderAccountModal() {
    const isCloud = signedIn();
    const avatar = state.profile.avatarUrl ? `<img src="${escapeHtml(state.profile.avatarUrl)}" alt="">` : `<span>${escapeHtml(profileInitials())}</span>`;
    return `<div class="modal-backdrop open" data-action="close-modal"></div><section class="account-modal" role="dialog" aria-modal="true"><button class="modal-x" data-action="close-modal" type="button">${miniIcon('x')}</button>${isCloud ? `<div class="account-hero"><div class="account-avatar">${avatar}</div><h2>${escapeHtml(state.profile.name || 'LiveDash user')}</h2><p>${escapeHtml(state.profile.email || 'Cloud account')}</p><span class="cloud-state">Cloud profile active</span></div><div class="account-stats"><div><strong>${(state.bookmarkSlots||[]).filter(s=>s.url).length}</strong><span>Shortcuts</span></div><div><strong>${(state.tasks||[]).filter(t=>t.status!=='done').length}</strong><span>Open tasks</span></div><div><strong>${state.pet?.score||0}</strong><span>Akita score</span></div></div><div class="auth-actions profile-actions"><button class="primary-action" data-action="refresh-cloud" type="button">Sync now</button><button data-action="sign-out" type="button">Sign out</button></div>` : `<div class="signin-hero"><div class="signin-logo"><img src="assets/icons/icon128.png" alt="LiveDash"></div><h2>Sign in to LiveDash</h2><p>Connect your cloud profile to sync shortcuts, tasks, notes, and Akita progress.</p></div><button class="google-signin" data-action="google-sign-in" type="button"><span class="google-mark">${icons.google}</span><span><strong>Continue with Google</strong><small>Secure cloud sync for LiveDash</small></span></button><div class="signin-divider"><span></span><em>or</em><span></span></div><label class="email-field"><span>Email address</span><input id="authEmail" type="email" placeholder="you@example.com"></label><button class="secondary-action" data-action="email-sign-in" type="button">Continue locally</button>`}</section>`;
  }
  function renderBookmarkModal() { const slot = (state.bookmarkSlots || []).find((b) => b.id === bookmarkEditingId) || {}; return `<div class="modal-backdrop open" data-action="close-modal"></div><section class="account-modal compact-modal"><button class="modal-x" data-action="close-modal" type="button">${miniIcon('x')}</button><h2>Bookmark slot</h2><label class="email-field"><span>Name</span><input id="bookmarkName" value="${escapeHtml(slot.label === 'Add site' ? '' : slot.label || '')}" placeholder="Gmail"></label><label class="email-field"><span>URL</span><input id="bookmarkUrl" value="${escapeHtml(slot.url || '')}" placeholder="https://example.com"></label><button class="primary-action" data-action="save-bookmark" type="button">Save shortcut</button></section>`; }
  function renderSettings() { return settingsOpen ? `<div class="modal-backdrop open" data-action="close-settings"></div><aside class="settings-drawer"><button class="modal-x" data-action="close-settings" type="button">${miniIcon('x')}</button><h2>Settings</h2><h3>Background</h3><div class="theme-grid">${['sky','mist','pearl','sunset','forest'].map((t) => `<button class="${state.settings.theme===t?'active':''}" data-action="theme" data-theme="${t}" type="button"><span class="theme-swatch ${escapeHtml(t)}"></span><strong>${escapeHtml(t[0].toUpperCase()+t.slice(1))}</strong></button>`).join('')}</div><h3>Data</h3><button data-action="export" type="button">Export backup</button><label class="import-label">Import backup<input id="importFile" type="file" accept="application/json"></label><button data-action="reset" type="button">Reset dashboard</button></aside>` : ''; }
  function renderCommand() { return commandOpen ? `<div class="modal-backdrop open" data-action="close-command"></div><section class="command-card"><input id="commandInput" placeholder="Type a command or search"><button data-route="home" type="button">${miniIcon('home')}Home</button><button data-route="apps" type="button">${miniIcon('apps')}Apps</button><button data-action="add-task" type="button">${miniIcon('task')}Add task</button><button data-action="google-sign-in" type="button">${miniIcon('google')}Google sign in</button></section>` : ''; }
  ...
  async function petAction(mode) { const pet = { ...(state.pet || {}) }; pet.mode = mode; pet.lastInteractionAt = new Date().toISOString(); if (mode === 'feed') { pet.energy = Math.min(100, Number(pet.energy || 70) + 8); pet.mood = 'Fed'; pet.score = Number(pet.score || 0) + 1; } if (mode === 'play') { pet.energy = Math.max(0, Number(pet.energy || 70) - 4); pet.mood = 'Playing'; pet.score = Number(pet.score || 0) + 2; } if (mode === 'rest') { pet.energy = Math.min(100, Number(pet.energy || 70) + 5); pet.mood = 'Resting'; } state.pet = pet; await save(); render(); setTimeout(async () => { if (state?.pet?.mode === mode) { state.pet.mode = 'idle'; state.pet.mood = 'Ready'; await window.LiveDashStore.setState(state); render(); } }, 2100); }
