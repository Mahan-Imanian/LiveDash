...
if(manifest.version !== "12.0.0") throw new Error("Manifest version must be 12.0.0");
...
for(const phrase of ["Capture current tab", "Edit layout", "Module library", "Activity", "Alerts", "Reports", "Source health"]){
  if(!app.toLowerCase().includes(phrase.toLowerCase())) throw new Error(`Missing expected app behavior marker: ${phrase}`);
}
console.log("LiveDash v12 extension validation passed");
