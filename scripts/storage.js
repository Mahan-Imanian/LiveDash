(function(global){
  ...
  async function getState(){
    ...
    if(!source || migrated || source.schemaVersion !== defaults.VERSION){
      appendActivity(merged, "migration", migrated ? "Storage migrated" : "Storage initialized", migrated ? "Older LiveDash data was upgraded to v9." : "Default LiveDash v9 state created.");
  ...
  async function exportState(){
    const state = await getState();
    appendActivity(state, "export", "Dashboard backup exported", "A versioned LiveDash v9 backup was created.");
    await saveState(state);
    return {
      product: "LiveDash",
      format: "livedash-v9-backup",
  ...
  async function resetState(){
    ...
    next.lastBackup = current;
    appendActivity(next, "reset", "Dashboard reset", "Default LiveDash v9 dashboard restored with a restore point.");
