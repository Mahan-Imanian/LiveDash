(function(global){
  ...
  async function getState(){
    ...
    if(!source || migrated || source.schemaVersion !== defaults.VERSION){
      appendActivity(merged, "migration", migrated ? "Storage migrated" : "Local workspace prepared", migrated ? "Older LiveDash data was upgraded to v12." : "Default tasks, captures, and source health were created.");
  ...
  async function exportState(){
    const state = await getState();
    appendActivity(state, "export", "Dashboard backup exported", "A versioned LiveDash v12 backup was created.");
    await saveState(state);
    return {
      product: "LiveDash",
      format: "livedash-v12-backup",
  ...
  async function resetState(){
    ...
    next.lastBackup = current;
    appendActivity(next, "reset", "Dashboard reset", "Default LiveDash v12 dashboard restored with a restore point.");
